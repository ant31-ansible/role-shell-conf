...
local prompt_user="${lblue}%n${std}"
local prompt_end="${lwhite}%(!.#.$)"

local prompt_ip="`ifconfig | grep 'inet addr'|cut -d':' -f 2|cut -d' ' -f 1 | grep -v 127.0.0.1 |sed ':a;N;$!ba;s/\n/ /g'`"

local prompt_host="${lcyan}%m${std}"
...
#╭
PROMPT="${white}${prompt_user}${purple}@${std}${prompt_host}${grey}[PROD] ${prompt_cwd} ${git_branch}${reset_color} ${prompt_end} %{${reset_color}%}
${purple}└> %{$reset_color%}"
# %(!.#.$) "
# ${reset_color} "
